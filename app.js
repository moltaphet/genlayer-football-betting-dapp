...
const connectWallet = async () => {
    ...
    } else { alert("Please install GenLayer extension!"); }
};
...
const sendBetTransaction = async (predictedWinner) => {
    ...
    if (amount < 10) { alert("Min bet is 10 GEN"); return; }
    ...
    try {
        ...
        addBetToHistory(predictedWinner, amount, txHash);
    ...
    row.innerHTML = `
        <td class="py-5 px-4">
            <div class="flex flex-col gap-1">
                ...
                <span class="text-[9px] text-blue-500 font-bold uppercase italic">TX: ${hash.slice(0, 8)}</span>
            </div>
        </td>
        <td class="py-5 px-4">
