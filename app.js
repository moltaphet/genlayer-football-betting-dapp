...
// WALLET ACTION (CONNECT & DISCONNECT)
...
// BET LOGIC (NO ALERT - LOGS ONLY)
const placeBet = (side) => {
    const amount = parseFloat(amountInput.value);
    if (!userAccount || amount <= 0) return;

    const data = matchData.find(m => m.id === currentMatchId);
    const team = side === 1 ? data.t1 : data.t2;
    
    // اینجا اطلاعات در کنسول ثبت می‌شود (F12)
    console.log(`Bet Placed: ${amount} GEN on ${team} (${selectedOdds}x)`);
};
